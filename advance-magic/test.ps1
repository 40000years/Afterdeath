...
& javac --release 21 -proc:none -encoding UTF-8 -cp $classPath -d $testClasses (Join-Path $moduleRoot 'tests/AccountingChecks.java') (Join-Path $moduleRoot 'tests/PackHttpChecks.java') (Join-Path $moduleRoot 'tests/GeyserCleanupChecks.java')
...
& java -cp ($testClasses + [System.IO.Path]::PathSeparator + $classPath) GeyserCleanupChecks
if ($LASTEXITCODE -ne 0) { throw 'Geyser duplicate cleanup tests failed.' }
