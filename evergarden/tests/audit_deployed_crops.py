...
for path in (geyser / 'custom_mappings').glob('*.json'):
    for base, entries in json.loads(path.read_text(encoding='utf-8-sig'))['items'].items():
...
with zipfile.ZipFile(geyser / 'packs/voidscape-bedrock.mcpack') as pack:
    ...
    for cid in ids:
        for stage in range(3):
            ...
            assert base == entry['model'] == 'minecraft:iron_helmet', name
            assert entry['predicate']['value'] == 'voidscape:' + name
            ...
            for cube in bone['cubes']:
                ...
                assert all(face['uv_size'] == [32, 32] for face in cube['uv'].values()), name
