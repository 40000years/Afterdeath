...
public final class VoidGenerator extends ChunkGenerator {
    ...
    private final Blueprint sanctumDark = Blueprint.sanctumDark();
    private final Blueprint sanctumAstral = Blueprint.sanctumAstral();
    private final Blueprint sanctumTime = Blueprint.sanctumTime();

    public VoidGenerator(long seed, DungeonLayout layout) {
        this.layout = layout;
        this.islands = new SimplexNoiseGenerator(seed);
        this.detail = new SimplexNoiseGenerator(seed ^ 721945L);
    }
    ...
    public void generateNoise(WorldInfo info, Random random, int cx, int cz, ChunkData data) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                for (var site : sites) {
                    if (Math.hypot((double) wx - site.x(), (double) wz - site.z()) < site.radius() + 18) {
                        structure = site;
                        break;
                    }
                }
        ...
        // Render sanctums
        for (var site : sites) {
            if (site.contains(cx * 16 + 8, cz * 16 + 8, 12)) {
        ...
        // Central Spawn Island Platform (Chunk -1 to 1)
        if (Math.abs(cx) <= 1 && Math.abs(cz) <= 1) {
            for (int x = 0; x < 16; x++) {
                for (int z = 0; z < 16; z++) {
                    int wx = cx * 16 + x, wz = cz * 16 + z;
                    if (Math.abs(wx) <= 8 && Math.abs(wz) <= 8) {
                        data.setBlock(x, 96, z, Material.DEEPSLATE_TILES);
                    }
                    if ((Math.abs(wx) == 8 && Math.abs(wz) <= 8) || (Math.abs(wz) == 8 && Math.abs(wx) <= 8)) {
                        data.setBlock(x, 97, z, Material.POLISHED_BLACKSTONE_WALL);
                    }
                    if (wx == 0 && wz == 0) {
                        data.setBlock(x, 96, z, Material.SEA_LANTERN);
                    }
                    if (wx == 0 && wz == 4) {
                        data.setBlock(x, 97, z, Material.LECTERN);
                    }
                    if (wx == 0 && Math.abs(wz) == 8) {
                        data.setBlock(x, 97, z, Material.AIR);
                    }
