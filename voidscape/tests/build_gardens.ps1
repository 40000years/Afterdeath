...
$classes = Join-Path $workspaceRoot 'voidscape/target/garden-test-classes'
...
$deps += @((Join-Path $PaperTestRoot 'versions/26.2/paper-26.2.jar'),(Join-Path $workspaceRoot 'voidscape.jar'),(Join-Path $workspaceRoot 'advance-magic.jar'))
...
@('name: GardensChecks','version: 1.0','main: GardensChecks',"api-version: '1.21'",'depend: [Voidscape, advance-magic]') | Set-Content -Encoding ASCII (Join-Path $classes 'plugin.yml')
& jar --create --file (Join-Path $workspaceRoot 'voidscape/target/gardens-checks.jar') -C $classes .
