...
public final class ItemFixChecks extends JavaPlugin implements Listener {
    ...
    void runChecks() throws Exception {
        ...
        var exact=(ShapedRecipe)Bukkit.getRecipe(new NamespacedKey(magic,"frost_nova_ni_c"));
        ...
        for(Spell spell:Spell.values()) {
            ItemStack fake=new ItemStack(Material.HEART_OF_THE_SEA);
            var meta=fake.getItemMeta();meta.setDisplayName("Core of "+magic.wands().coreTitle(spell));fake.setItemMeta(meta);
            result("fake_core_rejected_"+spell.id(),magic.wands().coreSpell(fake)==null);
            fake=new ItemStack(Material.CARROT_ON_A_STICK);meta=fake.getItemMeta();meta.setDisplayName(spell.title+" Wand");fake.setItemMeta(meta);
            result("fake_wand_rejected_"+spell.id(),magic.wands().spell(fake)==null);
        }
        ItemStack conflict=magic.wands().createCore(Spell.FROST_NOVA);
        var meta=conflict.getItemMeta();meta.getPersistentDataContainer().set(new NamespacedKey("voidscape","magic_core"),PersistentDataType.STRING,"meteor_strike");conflict.setItemMeta(meta);
        result("conflicting_core_rejected",magic.wands().coreSpell(conflict)==null);
        ItemStack old=new ItemStack(Material.HEART_OF_THE_SEA);meta=old.getItemMeta();meta.getPersistentDataContainer().set(new NamespacedKey("voidscape","magic_core"),PersistentDataType.STRING,"frost_nova");old.setItemMeta(meta);
        result("legacy_core_accepted",magic.wands().coreSpell(old)==Spell.FROST_NOVA);
        Material[] bases={Material.PRISMARINE_SHARD,Material.FLINT,Material.HONEY_BOTTLE,Material.SUGAR};
        String[] names={"Key Shard","Repair Stone","Void Walker Elixir","Astral Dust"};
        for(int i=0;i<bases.length;i++) {
            ItemStack fake=new ItemStack(bases[i]);meta=fake.getItemMeta();meta.setDisplayName(names[i]);fake.setItemMeta(meta);
            result("fake_relic_rejected_"+i,!garden.relics().isKeyShard(fake)&&!garden.relics().isRepairStone(fake)&&!garden.relics().isVoidElixir(fake)&&!garden.relics().isAstralDust(fake));
        }
        actor.setGameMode(GameMode.CREATIVE);
        for(Material base:List.of(Material.HEART_OF_THE_SEA,Material.CARROT_ON_A_STICK)) {
            var creative=new InventoryCreativeEvent(actor.getOpenInventory(),InventoryType.SlotType.QUICKBAR,36,new ItemStack(base));
            Bukkit.getPluginManager().callEvent(creative);
            result("plain_creative_allowed_"+base,!creative.isCancelled());
        }
        actor.setGameMode(GameMode.SURVIVAL);
        actor.getInventory().clear();
        actor.getInventory().setItemInMainHand(magic.wands().createCore(Spell.FROST_NOVA));
        actor.getInventory().setItem(1,new ItemStack(Material.NETHERITE_INGOT,4));
        actor.getInventory().setItem(2,new ItemStack(Material.NETHER_STAR,4));
        var table=world.getBlockAt(1,100,0);table.setType(Material.CRAFTING_TABLE);
        var interact=new org.bukkit.event.player.PlayerInteractEvent(actor,org.bukkit.event.block.Action.RIGHT_CLICK_BLOCK,actor.getInventory().getItemInMainHand(),table,org.bukkit.block.BlockFace.UP,EquipmentSlot.HAND);
        Bukkit.getPluginManager().callEvent(interact);
        result("table_opens_without_auto_consumption",interact.useInteractedBlock()!=Event.Result.DENY&&actor.getInventory().contains(Material.NETHERITE_INGOT,4)&&magic.wands().coreSpell(actor.getInventory().getItemInMainHand())==Spell.FROST_NOVA);
        actor.openWorkbench(null,true);
        var crafting=(CraftingInventory)actor.getOpenInventory().getTopInventory();
        ItemStack[] matrix=new ItemStack[9];for(int i=0;i<9;i++)matrix[i]=new ItemStack(Material.NETHER_STAR);matrix[4]=magic.wands().createCore(Spell.FROST_NOVA);crafting.setMatrix(matrix);
        var move=new InventoryClickEvent(actor.getOpenInventory(),InventoryType.SlotType.CRAFTING,5,ClickType.LEFT,InventoryAction.PICKUP_ALL);
        Bukkit.getPluginManager().callEvent(move);
        result("core_click_does_not_craft",!move.isCancelled()&&magic.wands().coreSpell(crafting.getMatrix()[4])==Spell.FROST_NOVA);
        actor.closeInventory();

    }
    void finish(){
        try{Files.write(Path.of("item-fix-result.txt"),results);}catch(Exception ignored){}
